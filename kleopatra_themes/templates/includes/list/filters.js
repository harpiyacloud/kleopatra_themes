function setup_list_filters() {
	...
	harpiya.call('harpiya.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = harpiya.utils.get_query_params()[df.fieldname];
			const f = harpiya.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(harpiya.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = harpiya.utils.make_query_string(query_params);
...
harpiya.ready(setup_list_filters);
