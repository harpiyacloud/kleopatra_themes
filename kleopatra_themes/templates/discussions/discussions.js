...
const setup_socket_io = () => {
	harpiya.socketio.init(window.socketio_port || "9000");
	harpiya.socketio.socket.on("publish_message", (data) => {
		publish_message(data);
	});
	harpiya.socketio.socket.on("update_message", (data) => {
		update_message(data);
	});
	harpiya.socketio.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const hide_actions_on_conditions = (template, owner) => {
	...
	harpiya.session.user != owner && $template.find(".dropdown").addClass("hide");
	return $template.prop("outerHTML");
};
